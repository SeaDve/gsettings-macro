...
/// an enum, it would generated a normal Rust enum with each enum
/// variant nick specified in the GSchema converted to pascal case.
/// The enum would implement both [`ToVariant`](glib::ToVariant)
/// and [`FromVariant`](glib::FromVariant), and also [`Clone`],
/// [`Hash`], [`PartialEq`], [`Eq`], [`PartialOrd`], [`Ord`]. On
/// the other hand, if it is a flag, it would generate bitflags
/// same as the bitflags generated by the [`bitflags`] macro with
/// each flags nick specified in the GSchema converted to screaming
/// snake case.
///
...
///
/// Note: The type has to implement both [`ToVariant`](glib::ToVariant)
/// and [`FromVariant`](glib::FromVariant) or it would fail to compile.
///
/// ### Default trait
///
