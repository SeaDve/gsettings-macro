...
/// as long as the type implements [`ToVariant`](gio::glib::ToVariant)
/// for argument type and [`FromVariant`](gio::glib::FromVariant)
/// for the return type. Usage would be further explained in the
/// next section.
///
/// Furthermore, it will automatically generate enums or flags
/// if specified in the GSchema file. If it is an enum, it would
/// generate a normal Rust enum with the variants of the nicks as
/// defined converted to pascal case. The enum would implement
/// both [`ToVariant`](gio::glib::ToVariant) and
/// [`FromVariant`](gio::glib::FromVariant). On the other hand,
/// if it is a flag, it would generate bitflags same as the bitflags
/// generated by the [`bitflags`](gio::glib::bitflags::bitflags)
/// macro.
///
